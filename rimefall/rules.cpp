...
static GameDefs g = {
	...
	0,	// WEATHER_EXISTS
	// MUST stay 0. Game::CheckVictory is the only per-turn ruleset hook, and runorders.cpp calls
	// it only when OPEN_ENDED is 0. Rimefall hangs the gateway rebuild, both invasion fronts and
	// the election off that hook, so setting this to 1 would leave a ruleset that still builds,
	// still runs, and is no longer the game. See docs/decisions/0011.
	0,	// OPEN_ENDED
	1,	// NEXUS_EXISTS
	0,	// CONQUEST_GAME
	...
	// MUST stay 0. This is what selects the curated gateway path in ARegionList::SetACNeighbors,
	// which stage 3 re-keys from terrain type to latitude band. See docs/decisions/0010 section 9.
	...
	0,	// MULTI_HEX_NEXUS
	0,	// ICOSAHEDRAL_WORLD
	// Rimefall is a single surface level. With no underworld, mountains and desert are the only
	// sources of mithril, rootstone and adamantium, which is why stage 2 must keep mountains in
	// every latitude band. See docs/decisions/0010 section 2.
	0,	// UNDERWORLD_LEVELS
	0,	// UNDERDEEP_LEVELS
	0,	// ABYSS_LEVEL
	90,	// TOWN_PROBABILITY
	...
	5,	// LAKES
	// One tapering continent, so no archipelago scatter. Stage 2 shapes the landmass in MakeLand.
	0,	// ARCHIPELAGO
