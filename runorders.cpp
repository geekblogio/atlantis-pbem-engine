...
void Game::RunOrders()
{
    ...
    for (auto ob = 0; ob < NOBJECTS; ob++) {
        ...
        logger::write("Running SACRIFICE Orders...");
        RunSacrificeOrders();
        break;
    }
...
void Game::Do1Assassinate(ARegion *r, Unit *u)
{
    ...
    for(const auto f : seers) {
        ...
        if (f->get_attitude(tar->faction->num) == AttitudeType::ALLY) {
            succ = 0;
            break;
        }
        ...
        for(const auto o : r->objects) {
            for(const auto u : o->units) {
                for(const auto fo : u->forgetorders) {
                    ...
                    u->event("Forgets " + SkillStrs(fo->skill) + ".", "forget");
                }
    ...
    for(const auto o : reg->objects) {
        for(const auto u : o->units) {
            if (u->taxing == TAX_TAX) {
                ...
                u->event("Collects $" + std::to_string(amt) + " in taxes in " + reg->short_print() + ".", "tax", reg);
                u->taxing = TAX_NONE;
            }
        }
    }
    ...
    for(const auto o : reg->objects) {
        for(const auto u : o->units) {
            if (u->taxing == TAX_PILLAGE) {
                ...
                u->event("Pillages $" + std::to_string(temp) + " from " + reg->short_print() + ".", "tax", reg);
                for(const auto f : facs) {
                    if (f != u->faction) f->event(u->name + " pillages " + reg->short_print() + ".", "tax", reg, u);
                }
            }
        }
    }
        ...
        for(const auto obj : r->objects) {
            for(const auto u : obj->units) {
                if (u->needed > 0) {
                    ...
                    for(const auto obj2 : r->objects) {
                        for(const auto u2 : obj2->units) {
                            if (u->faction != u2->faction && u2->GetAttitude(r, u) == AttitudeType::ALLY) {
                                int amount = u2->items.GetNum(item);
                                if (amount) {
                                    int eat = (u->needed + value - 1) / value;
                                    if (eat > amount) eat = amount;
                    ...
                    for(const auto obj2 : r->objects) {
                        for(const auto u2 : obj2->units) {
                            if (u->faction != u2->faction &&
                                u2->GetAttitude(r, u) == AttitudeType::ALLY) {
                                int amount = u2->items.GetNum(item);
                                if (amount) {
                                    int eat = (u->hunger + value - 1) / value;
                                    if (eat > amount) eat = amount;
...
int Game::DoGiveOrder(ARegion *r, Unit *u, GiveOrder *o)
{
    ...
    if ((o->item >= 0) && (ItemDefs[o->item].type & IT_SHIP)) {
        ...
        if (!u->CanSee(r, t) &&
            (t->faction->get_attitude(u->faction->num) < AttitudeType::FRIENDLY)) {
                u->error(ord + ": Nonexistant target (" + o->target->Print() + ").");
                return 0;
        }
        if (t->faction->get_attitude(u->faction->num) < AttitudeType::FRIENDLY) {
    ...
    // New RULE -- Must be able to see unit to give something to them!
    if (!u->CanSee(r, t) && (t->faction->get_attitude(u->faction->num) < AttitudeType::FRIENDLY)) {
        u->error(ord + ": Nonexistant target (" + o->target->Print() + ").");
        return 0;
    }
    ...
    if (o->item != I_SILVER && t->faction->get_attitude(s->faction->num) < AttitudeType::FRIENDLY) {
        u->error(ord + ": Target is not a member of a friendly faction.");
        return 0;
    }
    ...
    if (amt == -1) {
        ...
        notallied = 1;
        if (t->faction->get_attitude(u->faction->num) == AttitudeType::ALLY) {
            notallied = 0;
        }
